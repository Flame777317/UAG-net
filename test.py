...
import models
from models.common import config
from models.networks.mynet import A, AT, A_CDP, At_CDP, Poisson_noise_torch,Gaussian_noise_torch
...
def testing(network, val, save_img=config.para.save, manner='grey', device=config.para.device):
    """
    The pre-processing before TCS-Net's forward propagation and the testing platform.
    """
    recon_root = f"./reconstructed_images_256_{config.para.rate}_{config.para.alpha}"
    if not os.path.isdir(recon_root):
        os.mkdir(recon_root)
    datasets =  ["PRTEST128unt","PRTEST128nt"]#["Set11"] "PRTEST128unt","PRTEST128nt","PRTEST256unt","PRTEST256nt"#["PRTEST256unt","PRTEST256nt","PRTEST128unt","PRTEST128nt"]#,"PRTEST256unt","PRTEST256nt"] #,"McM18", "General100", "Urban100", "LIVE29", "OST300","Set14"]# 'CIFAR10', "McM18", "General100", "Urban100", "LIVE29", "OST300","original", "BSD68"
    with torch.no_grad():
        for one_dataset in datasets:
            ...
            for _, _, images in os.walk(f"{test_dataset_path}/"):
                for one_image in images:
                    ...
                    b = Poisson_noise_torch(A_CDP(batch_x, SamplingRate=config.para.rate,
                                            mask=mask), alpha=config.para.alpha)
...
if __name__ == "__main__":
    ...
    net = models.AUV_Net(rate=config.para.rate).eval().to(device)
